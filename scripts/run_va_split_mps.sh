...
VA_SPLIT_MAX_VLM_WAIT_MS="${VA_SPLIT_MAX_VLM_WAIT_MS:-1.0}"
...

{
  ...
  echo "MAX_PREFIX_SLOTS=${MAX_PREFIX_SLOTS:-}"
  echo "PREFIX_SLOT_MULTIPLIER=${PREFIX_SLOT_MULTIPLIER:-}"
  echo "EFFECTIVE_PREFIX_SLOTS=${EFFECTIVE_PREFIX_SLOTS}"
...
echo "  split: max_ae_batch=${MAX_AE_BATCH_SIZE} max_vlm_batch=${MAX_VLM_BATCH_SIZE} max_vlm_wait_ms=${MAX_VLM_WAIT_MS} prefix/admission=${EFFECTIVE_PREFIX_SLOTS} ae_sm=${AE_SM_PERCENT} vlm_sm=${VLM_SM_PERCENT}"
if [[ "${NEEDS_MPS}" -eq 1 ]]; then
