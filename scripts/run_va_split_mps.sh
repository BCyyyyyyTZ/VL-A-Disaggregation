...
VA_SPLIT_MAX_AE_BATCH_SIZE="${VA_SPLIT_MAX_AE_BATCH_SIZE:-16}"
...
if [[ "${RUN_MODE}" == "smoke" ]]; then
  ...
  REQUEST_RATE_HZ="${REQUEST_RATE_HZ:-4}"
...
echo "  CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"
echo "  workload: requests=${NUM_REQUESTS} rate=${REQUEST_RATE_HZ}/s max_inflight=${MAX_INFLIGHT} batch_size=${BATCH_SIZE} steps=${NUM_STEPS} warmup=${WARMUP_REQUESTS}"
